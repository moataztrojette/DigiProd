...
const Calendrier = () => { 
    ...
    const eventsCalendar = ()=>{
        const res = calendar.map(event=>{
            return {
                 title :event.title +" "+event.pourcentage+"%    "+event.heureDebut+"-"+event.heureFin,
                 start : event.start,
                 end : event.end,
    ...
    const MyValueInput = (event) => {
      ...
      const deletedTache   = async (id)=>{
        ...
        await axios.delete("http://localhost:4000/api/calendar/delete/" + id)
        .then((verife)=>{
    ...
    const handleFormSubmit = async(event)=>{
            ...
            const data =await axios.post("http://localhost:4000/api/calendar/post",valuesInput)
...
<div className="auth-form-light text-left p-5">
          ...
          >
            ...
            <input type="range" id="volume" name="pourcentage"
            min="0" max="100" onChange={MyValueInput}></input>
            </div>


            <h5 className="auth-link text-black">Heure début  </h5>
            <div className="form-group">
            <input type="time" id="appt" name="heureDebut"
            min="07:00" max="00:00" required onChange={MyValueInput}></input>
                    </div>
...
<h3>Calendrier</h3>
  ...
  <div className="serhceInput">
    ...
    <button type="button"
 className="btn btn-primary-color_inv" onClick={() => setModalIsOpen(true)}>Nouvelle Tâche + </button>
  </div>
</div>
