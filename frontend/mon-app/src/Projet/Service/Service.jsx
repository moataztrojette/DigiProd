...
const Service = () => { 
    ...
    useEffect(()=>{
        const data = axios.get("http://localhost:4000/api/service/findall").then((service)=>{
    ...
    const handleFormSubmit = async(event)=>{
        try{
            ...
            const data =await axios.post("http://localhost:4000/api/service/post",valuesInput)
    ...
    const deletedService = async (id)=>{
        ...
        await axios.delete("http://localhost:4000/api/service/deleted/" + id)
        .then((verife)=>{
      ...
      const  handleFormSubmitUpdate =  async (event)=>{
        event.preventDefault()
      const datatService = await axios.put("http://localhost:4000/api/service/update/" + valuesInput_update._id,valuesInput_update)  
...
<div className="auth-form-light text-left p-5">
      ...
      <Modal
        ...
        shouldCloseOnOverlayClick={false}
        onRequestClose={() => {setModalUpdateIsOpen({
          open:false,
          info : {}
        }); setValues_update({})}}
        style={{
...
<h3>Service</h3>
  ...
  <div className="serhceInput">
    ...
    <button type="button"
 className="btn btn-primary-color_inv" onClick={() => setModalIsOpen(true)}>Nouveau Service + </button>
  </div>
</div>

<div className="row">

{service.length>0  ? <div className="title_Service">
    <ul>
        <li>Nom Service</li>
        <li>Description</li>
        <li>Contact</li>
     ...
     {service.map((ser)=>(
        ...
        <img src="/image/icons/delete_icons.png" alt=""  onClick={() => {
                      Swal.fire({
                        title: "Êtes - vous sûr ?",
                        text: "",
                        icon: "warning",
                     ...
                     />
        <img src="/image/icons/update_icons.png" onClick={() => {setModalUpdateIsOpen({
          open:true,
          info : ser
        }); setValues_update(ser)}} alt="" />
        </div>
    </ul>
